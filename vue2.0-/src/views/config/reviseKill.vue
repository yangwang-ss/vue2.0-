...
//				cycle: '', //批次周期
		...
		methods: {
			...
			//技能信息回显
			skillMessage(id){
				this.axios.post('/config/getSkillInfo.do', {
					"id":id                //类型：String  必有字段  备注：技能Id
				}).then((res) => {
					if(res.data.code==0){
						this.skillName = res.data.data.name;
						this.skillDes = res.data.data.description;
						this.treeSelected = res.data.data.id;
						this.picture.src = res.data.data.url;
					}else{
			...
			updateSkillList() {
				var that = this;
				this.axios.post('/config/updateSkill.do', {
					...
					"id": that.treeSelected //类型：String  必有字段  备注：技能Id
				}).then((res) => {
					if(res.data.code == 0) {

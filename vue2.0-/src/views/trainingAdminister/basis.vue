<template>
	<div class="main">
		...
		<Row class="main-content">
			...
			<input placeholder="请输入" class="inputText" v-model="trainingName">
			</Col>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addEdit(index, id) {
				switch(index) {
					case 0:
						{
							...
							this.trainingAdminister.title = "新建基础培训";
							this.picture2.src = '';
					...
					case 1:
						{
							this.editeList = true;
							this.trainingAdminister.title = "编辑基础培训";
							this.editId = id;
			...
			submit() {
				// 新建提交
				if(this.trainingAdminister.title === "新建基础培训") {
					this.logunMdarin.map((item) => {
						item.previewTime = this.previewTime;
						item.remainingTime = this.remainingTime;
					});
					if(this.trainingAdminister.name && this.examinationModel && this.picture2.src) {
						let content = {
				...
				this.axios(option).then(function(response) {
					...
					if(result.code == '0') {
						...
						let infoForm = {
							...
							srcId: result.data.id
						};
						_this.picture1.backId.push(infoForm)
						let fileMess = {
							name: result.data.name,
							id: result.data.srcId
						}

<template>
  <div class="add-main">
    ...
    <div class="add-name">
      ...
      <div class="add-nameContent">
        <div class="nameContent-list listArea" v-for="(item,index) in foreign.answer">
          ...
          <span @click="addForeignAnswer()"><i data-v-0c04591f="" class="icon iconfont icon-spread"
    ...
    <div class="add-name">
      ...
      <div>
        <span class="span smallBtnDefalt" @click="enablement(0)"
              :class="{'enablementActive':enablementStatus}">启用</span>
        <span class="span smallBtnDefalt" @click="enablement(1)"
              :class="{'enablementActive':!enablementStatus}">停用</span>
      </div>
      <div class="add-radio">
    ...
    <v-bounces v-if="bouncesHide" @emitClickPopup="bouncesHide=false">
      <v-associated @shutDown="bouncesHide=false"
                    ...
                    questionAndknowledge="question"
                    @preservationKnowledge="preservationKnowledge()"
                    @addKnowledge="addKnowledge($event)">
  ...
  export default{
        ...
        this.axios.post("/knowledge/question/modify/"+this.questionId+".do", {
          ...
          if (res.data.code === "0") {
            this.$Message.success({
              ...
              duration: 0.5,
              onClose: () => {
                this.$emit("succeedBit",res.data.data.value);
              }
            })
          } else {
            return;
    ...
    watch: {
      ...
      "identification.add"(){
        ...
        this.question.similarity = [{value: ""}];
        this.foreign.answer = [{value: ""}];
        this.internal.answer = [{value: ""}];
